function Chunk(chunkX, chunkZ)
    ...
    newChunk.blockData.size = 0

    newChunk.blockData.createBlock = function(this, blockID, XPos, YPos, ZPos)
        this.blockData[this.blockData.size] = {
            block = blockID,
            X = XPos,
            Y = YPos,
            Z = ZPos
        ...
        this.blockData.size = this.blockData.size + 1
    end

    newChunk.blockData.draw = function(this)
        local BD = this.blockData
        for index = 0, BD.size do
                    ...
                    if this:getBlock(X, Y, Z) ~= AIR then
                        if X < CHUNK_SIZE - 1 then
                            if this:getBlock(X + 1, Y, Z) == AIR then
                                BD:createBlock(this:getBlock(X, Y, Z), worldPosX + X, Y, worldPosZ + Z)
                                goto continue
                            end
                        end
                        if Y <= this.highestPoint then
    ...
    newChunk.needToUpdate = true

    newChunk.loadModels = function(this)
        if this.needToUpdate then
            this:updateModels()
